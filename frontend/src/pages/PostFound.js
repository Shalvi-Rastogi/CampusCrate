...
export default function PostFound() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append claimQuestion if present
      if (form.claimQuestion) {
      ...
      if (form.photoUrl) {
        formData.append('photo', form.photoUrl);
      }
      
      await axios.post('/items', formData);
      navigate('/dashboard/found');
    } catch (error) {
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'Failed to post item. Please try again.';
